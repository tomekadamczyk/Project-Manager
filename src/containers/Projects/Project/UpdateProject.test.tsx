...
describe('should test Projects render', function() {
    ...
    test('test', async () => {
        ...
        const button = await screen.findByRole('submit');
        expect(button).toBeInTheDocument()
        await userEvent.click(button)
        
        expect(await screen.findByText('update loading')).toBeInTheDocument();
        expect(store.get('Project', '1', 'name')).toBe('1ew');
    ...
    test('tes2', async () => {
            ...
            const resolvers = () => ({
                Query: {
                    // projects: () => [project],
                    project: (obj: any, { id }: any, ctx: any, info: any) => {
                        const ob = store.get('Project', id, 'name');
            ...
            await waitFor(() => expect(resolvers().Mutation.updateProject).toHaveBeenCalled());
        })
